...
<section>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="entityForm">
            ...
            <div class="row">
              <div class="col-md-5 mb-3"><input type="text" id="proveedor" name="proveedor" class="form-control" placeholder="Proveedor"></div>
              <div class="col-md-3 form-check mb-3"><input class="form-check-input" style="margin-left: 30px;" type="checkbox" id="sector_publico" name="sector_publico"><label class="form-check-label" for="sector_publico">Sector Público</label></div>
              <div class="col-md-3 form-check mb-3"><input class="form-check-input" style="margin-left: 30px;" type="checkbox" id="sector_economico" name="sector_economico"><label class="form-check-label" for="sector_economico">Sector Económico</label></div>
            </div>
            <div class="mb-3"><input type="text" id="tipo_institucion" name="tipo_institucion" class="form-control" placeholder="Tipo de institución"></div>
            <div class="row">
              <div class="col-md-6 mb-3">
                <label class="form-label">Departamento</label>
                <select id="departamento" name="departamento" class="form-select"><option value="">Seleccione Departamento</option></select>
              </div>
              <div class="col-md-6 mb-3">
            ...
            <div class="mb-3 hide">
              ...
              <select id="banco" name="banco" class="form-select"><option value="">Seleccione Banco</option></select>
            </div>
          </form>
        </div>
  ...
  <div class="modal fade" id="sedeModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Sedes del Benefactor</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <form id="sedeForm" class="mb-3">
            ...
            <div class="mb-3"><input type="text" id="nit" name="nit" class="form-control" placeholder="NIT"></div>
            <div class="mb-3"><input type="text" id="sede_nombre" name="nombre" class="form-control" placeholder="Nombre Sede"></div>
            <div class="row">
              <div class="col-md-6 mb-3">
                <label class="form-label">Departamento</label>
                <select id="sede_departamento" name="departamento" class="form-select"><option value="">Seleccione Departamento</option></select>
              </div>
              <div class="col-md-6 mb-3">
                <label class="form-label">Municipio</label>
                <select id="sede_municipio" name="municipio" class="form-select"><option value="">Seleccione Municipio</option></select>
              </div>
            </div>
...
$(document).ready(function () {
  var baseURL    = '../../controllers/configuracion/benefactor/';
  var sedeURL    = '../../controllers/configuracion/benefactor_sede/';
  ...
  $('#btnCreate').on('click', () => {
    $('#modalTitle').text('Crear Benefactor');
    $('#entityForm')[0].reset();
    loadDepartamentos('#departamento');
  ...
  table.on('click', '.edit', function () {
    ...
    $.getJSON(baseURL + 'get.php', { id }, r => {
      var d = r.data;
      $('#modalTitle').text('Editar Benefactor');
      $('#id').val(d.id);
  ...
  table.on('click', '.delete', function () {
    var id = $(this).data('id');
    Swal.fire({ title: '¿Eliminar benefactor?', showCancelButton: true })
  ...
  let currentBenefactorId = null;

  $('#sedeModal').on('shown.bs.modal', function () {
    loadDepartamentos('#sede_departamento');
    loadMunicipios('', '#sede_municipio');

    $.getJSON(sedeURL + 'fetch_by_benefactor.php', { benefactor_id: currentBenefactorId }, r => {
  ...
  table.on('click', '.sedes', function () {
    currentBenefactorId = $(this).data('id');
    $('#sedeForm')[0].reset();
    $('#sede_id').val('');
    $('#sedeList').empty();
  ...
  $('#btnAddSede').off().on('click', function () {
    const data = $('#sedeForm').serialize() + '&benefactor_id=' + currentBenefactorId;
      ...
      if (r.success) {
        $.getJSON(sedeURL + 'fetch_by_benefactor.php', { benefactor_id: currentBenefactorId }, r => {
          ...
          r.data.forEach(s => {
            ul.append(
              `<li class="list-group-item d-flex justify-content-between align-items-center">
                ${s.codigo} - ${s.nombre} (${s.nit}) – ${s.departamento} / ${s.municipio}
                <div>
                  <button
                    class="btn btn-sm btn-warning edit-sede"
                    data-id="${s.id}"
          ...
          r.data.forEach(s => {
            ul.append(
              `<li class="list-group-item d-flex justify-content-between align-items-center">
                ${s.nombre} (${s.nit}) - ${s.departamento}/${s.municipio}
                <div>
                  <button class="btn btn-sm btn-warning edit-sede" data-id="${s.id}" data-codigo="${s.codigo}" data-nit="${s.nit}" data-nombre="${s.nombre}" data-departamento="${s.departamento}" data-municipio="${s.municipio}">✎</button>
                  <button class="btn btn-sm btn-outline-danger remove-sede" data-id="${s.id}">×</button>
                </div>
              </li>`
            );
  ...
  function loadMunicipios(depto, selectId, callback) {
    var sel = $(selectId).empty().append('<option value="">Seleccione Municipio</option>');
    if (!depto) {
      if (callback) callback();
      return;
    }
    $.getJSON('../../controllers/configuracion/util/fetch_municipios.php', { departamento: depto }, r => {
