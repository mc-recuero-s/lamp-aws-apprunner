<?php
  ...
  if( isset($_POST['benefactor']) ){
    ...
    $query .= "institucion = ".$_POST["benefactor"]." AND ((estado = 1 OR estado = 3) OR estado Is NULL)";
  }else{
    // $query="SELECT * FROM entrada WHERE ((estado <> 3 AND estado <> 4 AND estado <> 2) OR estado Is NULL)";
    $query="SELECT * FROM entrada WHERE ((estado = 1 OR estado = 3) OR estado Is NULL)";
  }
  // echo $query;
  ...
  foreach ($entradas as $entrada){
    ...
    $where = "";
    $query="SELECT l.id as id, l.id_entrada, l.id_bodega, l.individual, l.individual_cantidad, l.cantidad, l.unidad, l.categoria, l.lote, l.producto, l.vencimiento, l.estado, l.estado2 ";
