$(document).ready(function () {
    ...
    $.ajax({
        type: "POST",
        url: uriControllers + "certificacion/tesoreria-historial.php",
    })
    .done(function (data, textStatus, jqXHR) {        
        ...
        certificaciones.forEach(function (certificacion) {            
            ...
            if (certificacion.estado == "3") {
                buttons = buttons + '<a class="button btn-editar" title="Editar" target="_parent" href="certificacion-efectivo.php?id=' + certificacion.id + '"></a><div class="button btn-delete" title="Eliminar"></div>';
                }
            if (certificacion.estado == "5") {
                buttons = buttons + '<a class="button btn-editar" title="Editar" target="_parent" href="certificacion-efectivo.php?id=' + certificacion.id + '">';
            ...
            if (result.value || result.value == "") {
                ...
                $.ajax({
                    ...
                    .done(function (data, textStatus, jqXHR) {
                        ...
                        archivos = archivos + '<a href="' + './soportes/certificacion/' + certificacion.id + '_firmado.pdf" target="_blank" class="ver-btn button btn-firmado" title="Certificado con Firma - Presiona para abrir"></a>';
                        table.cell(index, 4).data(archivos).draw();
                        table.cell(index, 5).data("<div class='text-alert'>Rechazada por firma</div>").draw();
                        table.cell(index, 6).data('<div class="ver-btn button btn-ver" title="Ver">' + crearHoverVer(certificacion).html() + '</div>').draw();
                    });
            }
        });
